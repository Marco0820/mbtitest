...
export function Questionnaire({ questions }: Props) {
  ...
  const handleNextPage = () => {
    if (currentPage < totalPages) {
      ...
      router.push(`/test/results?${queryString}`);
    }
  };
      ...
      <div className="w-full max-w-4xl mx-auto py-12" ref={topRef}>
          ...
          {currentQuestions.map((question, index) => (
            ...
            >
              <p className="text-2xl font-semibold text-gray-800 mb-8 leading-relaxed">
                {(currentPage - 1) * questionsPerPage + index + 1}. {t_general(question.id as any)}
              </p>
              <div className="flex justify-center items-center space-x-4 md:space-x-6">
                <span className="text-lg font-bold text-green-600">{t_general('question_option_agree')}</span>
                {optionStyles.map((style, i) => (
                  <button
                    ...
                    className={`${style.size} ${style.bg} rounded-full transition-transform duration-200 ease-in-out hover:scale-110 focus:outline-none focus:ring-4 focus:ring-offset-2 ${
                      answers[question.id] === style.value ? 'ring-4 ring-purple-500 ring-offset-2' : 'ring-transparent'
                    }`}
                    aria-label={`Option ${style.value}`}
                  />
                ))}
                <span className="text-lg font-bold text-red-600">{t_general('question_option_disagree')}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {isLastPage && (
          <div className="mt-16 text-center">
            ...
            >
              <option value="" disabled>{t_general('gender_select')}</option>
              <option value="male">{t_general('male')}</option>
              <option value="female">{t_general('female')}</option>
              <option value="other">{t_general('other')}</option>
            </select>
          </div>
        )}
        ...
        <div className="flex justify-center items-center mt-8 space-x-6">
          ...
          >
            {t_general('previous_page')}
          </button>
          <button
            onClick={handleNextPage}
            disabled={isLastPage ? !canFinish : !areAllQuestionsOnPageAnswered}
          ...
          >
            {isLastPage ? t_general('finish_test') : t_general('next_page')}
          </button>
        </div>
      </div>
      
      <aside className="fixed top-1/2 -translate-y-1/2 right-8 hidden lg:flex flex-col items-center space-y-4">
        <h4 className="font-semibold text-lg text-gray-800">{t_test('progress')}</h4>
