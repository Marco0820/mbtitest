...
import { PersonalityHeaderRTL } from './PersonalityHeaderRTL';
...
const ContentItemRenderer: React.FC<{ item: ContentItem }> = ({ item }) => {
    ...
    switch (item.type) {
        case 'paragraph':
            return <p className="mb-4 text-right">{item.text}</p>;
        case 'subheading':
            return <h3 className="text-2xl font-bold mt-10 mb-4 text-gray-800 dark:text-gray-200 text-right">{item.text}</h3>;
        case 'illustration':
            return (
        ...
        case 'pullquote':
            return (
                <blockquote className="my-6 p-4 bg-gray-100 dark:bg-gray-800 border-r-4 border-gray-300 dark:border-gray-600 rounded-l-lg">
                    <p className="italic text-gray-600 dark:text-gray-400 text-right">"{item.text}"</p>
                </blockquote>
            );
        default:
            return null;
...
// --- MAIN COMPONENT --- //
interface PersonalityDetailRTLProps {
  type: string;
  locale: string;
}

const PersonalityDetailRTL: React.FC<PersonalityDetailRTLProps> = ({ type, locale }) => {
  ...
  const renderStandardSection = (section: ContentBlock) => {
    if (section.content) {
        return (
            <>
              {section.quote && (
                <blockquote className="my-6 p-4 border-r-4 border-purple-400 bg-purple-50 dark:bg-gray-800 rounded-l-lg">
                  <p className="text-xl italic text-gray-700 dark:text-gray-300 text-right">"{section.quote}"</p>
                </blockquote>
              )}
              <div className="prose prose-lg max-w-none dark:prose-invert text-gray-700 dark:text-gray-300 leading-relaxed space-y-4 text-right">
    ...
    if (section.text) {
        ...
        return (
            <>
              {introQuote && (
                <blockquote className="my-6 p-4 border-r-4 border-purple-400 bg-purple-50 dark:bg-gray-800 rounded-l-lg">
                  <p className="text-xl italic text-gray-700 dark:text-gray-300 text-right">"{introQuote}"</p>
                </blockquote>
              )}
              <div className="prose prose-lg max-w-none dark:prose-invert text-gray-700 dark:text-gray-300 leading-relaxed space-y-4 text-right">
                  {paragraphs}
  ...
  const renderStrengthsWeaknessesSection = (section: StrengthsWeaknessesBlock) => (
    <div className="grid md:grid-cols-2 gap-x-12 gap-y-8">
      <div className="text-right">
        <h3 className="text-2xl font-semibold text-green-600 mb-6 flex items-center flex-row-reverse">
      ...
      <div className="text-right">
        <h3 className="text-2xl font-semibold text-red-600 mb-6 flex items-center flex-row-reverse">
  ...
  return (
    <article>
      <BackToTopButton />
      <PersonalityHeaderRTL type={type} name={data.name || ''} title={data.title || ''} subtitle={data.subtitle || ''} />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="lg:grid lg:grid-cols-12 lg:gap-x-12">
          ...
          <main className="lg:col-span-9 lg:col-start-1 lg:row-start-1">
            {finalSections.map(section => {
              if (!section.data) return null;
              return (
                <section key={section.id} id={section.id} className="mb-12 scroll-mt-24">
                  <h2 className="text-3xl font-bold mb-6 text-gray-800 dark:text-gray-100 border-r-4 border-purple-500 pr-4 text-right">
                    {section.data.title}
                  </h2>
...
export default PersonalityDetailRTL; 